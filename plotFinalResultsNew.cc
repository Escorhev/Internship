...
void plotFinalResults_oneStyle(bool doPrompt) {
  ...
  */
  ...
  histMC->SetLineColor(kgreen);
  histMC->SetLineWidth(2);
  histMC->SetMarkerColor(kgreen);
  ...
  for (int iBin = 0; iBin < nBinZ_reco; iBin++) {
    ...
    y[iBin] = histPP->GetBinContent(bin);
    yMC[iBin] = histMC->GetBinContent(bin);
    exl[iBin] = x[iBin] - dist - prec;
    //cout<<x[iBin]<<" and "<<dist<<" and "<<prec<<endl;
    exh[iBin] = exl[iBin];
    eyl[iBin] = graphPP->GetErrorYlow(bin);
    eylMC[iBin] = histMC->GetBinError(bin);
    eyh[iBin] = graphPP->GetErrorYhigh(bin);
    eyhMC[iBin] = histMC->GetBinError(bin);
  }
  ...
  graphZ->SetLineColor(kpink);
  graphZ->SetMarkerColor(kpink);
  ...
  graphZMC->SetLineColor(kgreen);
  graphZMC->SetLineWidth(2);
  graphZMC->SetMarkerColor(kgreen);
  ...
  axisHist2->GetYaxis()->SetRangeUser(0, histPP->GetMaximum()*1.2);
  if (plotMC) {
    axisHist2->GetYaxis()->SetTitle("1/N dN/dz");
    axisHist2->GetYaxis()->SetRangeUser(0, histMC->GetMaximum()*1.2);
  }
  axisHist->Draw();
  if (plotMC) {
    graphZMC->Draw("same e1");
    graphZMC->Draw("same hist");
  }
...
TGraphAsymmErrors* systUncertaintyHistAll(bool doPbPb, bool doPrompt, TH1D* nominalHist) {
  ...
  string systLabels [] = {"Regularization","Prior","Transfer matrix stat."};
  int nSyst = sizeof(systNames)/sizeof(systNames[0]);
  Double_t x[4] = {0,0,0,0};
  Double_t x2[4] = {0,0,0,0};
  Double_t y[4] = {0,0,0,0};
  Double_t exl[4] = {0,0,0,0};
  ...
  TLatex *  text1 = new TLatex(0.2 , 0.79, "#Upsilon (1S)");
  text1->SetNDC();
  text1->SetTextFont(42);
  text1->SetTextSize(0.044);
  text1->SetLineWidth(2);
  ...
  text3->Draw("same");  
  */
  TLatex *  text4 = new TLatex(0.2 , 0.75, "|#eta_{Jet}| < 2");
  text4->SetNDC();
  text4->SetTextFont(42);
  text4->SetTextSize(0.035);
  text4->SetLineWidth(2);
