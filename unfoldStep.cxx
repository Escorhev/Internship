...
void unfold(bool doPrompt = true, bool doPbPb = true, Int_t iterMax = 8, Int_t stepNumber = 1) {
  ...
  for(Int_t iter = iterMin; iter<=iterMax; iter++) {
    ...
    cout<<hFolded[iter-iterMin]->GetNbinsX()<<" nbBins"<<endl;
  ...
  for(Int_t i = 0; i<nBinJet_gen; i++) {
    ...
    /*
    TH1D *gTruth1DX = dynamic_cast<TH1D*>(hReco[0]->ProjectionX("TX", 0, -1, "E"));
    TH1D *gTruth1DY = dynamic_cast<TH1D*>(hReco[0]->ProjectionY("TY", 0, -1, "E"));
    ...
    can4->cd(2);
    gTruth1DY->Draw("EP");
    can4->SaveAs("/home/llr/cms/henderson/UpsUnfolding/Unfolding/mcUnf/code/debughReco0Normal.png");

    TH1D *gTruth2DX = dynamic_cast<TH1D*>(hFolded[0]->ProjectionX("TX", 0, -1, "E"));
    TH1D *gTruth2DY = dynamic_cast<TH1D*>(hFolded[0]->ProjectionY("TY", 0, -1, "E"));
    ...
    can5->cd(2);
    gTruth2DY->Draw("EP");
    can5->SaveAs("/home/llr/cms/henderson/UpsUnfolding/Unfolding/mcUnf/code/debughFolded0Normal.png");
    */
    for(Int_t iter = iterMin; iter<=iterMax; iter++) {
  ...
  TH1D *cTruth1DX = dynamic_cast<TH1D*>(hReco[1]->ProjectionX("TX", 0, -1, "E"));
  TH1D *cTruth1DY = dynamic_cast<TH1D*>(hReco[1]->ProjectionY("TY", 0, -1, "E"));
  ...
  can6->cd(2);
  cTruth1DY->Draw("EP");
  can6->SaveAs("/home/llr/cms/henderson/UpsUnfolding/Unfolding/mcUnf/code/debughReco1Normal.png");
