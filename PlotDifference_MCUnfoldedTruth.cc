...
void plot(bool doPrompt = false, bool doPbPb = true){
  ...
  hZMeas->SetStats(0);
  hZMeas->SetTitle(Form("Histogramme des z^2 pour 5 iterations"));
  
  hZMeas->GetXaxis()->SetTitle("z^2");
  hZMeas->GetYaxis()->SetTitleOffset(1.6);
  //hZMeas->GetYaxis()->SetTitle(Form("Nombre d'évènements"));
  ...
  legend->AddEntry(hZMeas, "post-det","ep");
  legend->AddEntry(hZTrue, "generees","ep");
  legend->AddEntry(hZUnf_SI1, "1 SI","ep");
  legend->AddEntry(hZUnf_SI2, "2 SI","ep");
  legend->AddEntry(hZUnf_SI3, Form("%d SI",iterFinal-1),"ep");
  legend->AddEntry(hZUnf_SI4, Form("%d SI",iterFinal),"ep");
  ...
  hZUnf_SI1_diffMeas->GetXaxis()->SetTitle("z^2");
  ...
  mycan1->SaveAs(Form("%s/mcUnf/plots/unf_mc_%s_%s_jetR%d_diffMeasured_%dz%dptBins%dz%dptMeasBins%s_%dIter%dSIter%s.pdf",unfPath.c_str(),doPbPb?"PbPb":"PP",doPrompt?"prompt":"nonprompt", (int) (jetR*10), nBinZ_gen, nBinJet_gen, nBinZ_reco, nBinJet_reco, caseTag.c_str(),nIter,iterFinal, dataDist?"dataDist":""));
  mycan1->SaveAs(Form("%s/mcUnf/plots/unf_mc_%s_%s_jetR%d_diffMeasured_%dz%dptBins%dz%dptMeasBins%s_%dIter%dSIter%s.png",unfPath.c_str(),doPbPb?"PbPb":"PP",doPrompt?"prompt":"nonprompt", (int) (jetR*10), nBinZ_gen, nBinJet_gen, nBinZ_reco, nBinJet_reco, caseTag.c_str(),nIter,iterFinal, dataDist?"dataDist":""));

  hZMeas_diffTruth->SetTitle(Form("Ecart aux donnees generees pour 5 iterations"));

  hZUnf_SI1_diffTruth->SetStats(0);
  hZUnf_SI1_diffTruth->GetYaxis()->SetTitleOffset(1.6);
  hZMeas_diffTruth->GetYaxis()->SetRangeUser(1.2*hZUnf_SI3_diffTruth->GetMinimum(),1.2*hZMeas_diffTruth->GetMaximum());
  hZMeas_diffTruth->GetXaxis()->SetTitle("z^2");
  ...
  mycan1->SaveAs(Form("%s/mcUnf/plots/unf_mc_%s_%s_jetR%d_diffTruth_%dz%dptBins%dz%dptMeasBins%s_%dIter%dSIter%s.pdf",unfPath.c_str(),doPbPb?"PbPb":"PP",doPrompt?"prompt":"nonprompt", (int) (jetR*10), nBinZ_gen, nBinJet_gen, nBinZ_reco, nBinJet_reco, caseTag.c_str(),nIter,iterFinal, dataDist?"dataDist":""));
  mycan1->SaveAs(Form("%s/mcUnf/plots/unf_mc_%s_%s_jetR%d_diffTruth_%dz%dptBins%dz%dptMeasBins%s_%dIter%dSIter%s.png",unfPath.c_str(),doPbPb?"PbPb":"PP",doPrompt?"prompt":"nonprompt", (int) (jetR*10), nBinZ_gen, nBinJet_gen, nBinZ_reco, nBinJet_reco, caseTag.c_str(),nIter,iterFinal, dataDist?"dataDist":""));
}

void PlotDifference_MCUnfoldedTruth(){
